...
function Twins:OnCombatStart(delay)
	self:StartStatusBarTimer(315 - delay, "Enrage", "Interface\\Icons\\Spell_Shadow_UnholyFrenzy") 
	self:ScheduleAnnounce(135 - delay, DBM_GENERIC_ENRAGE_WARN:format(3, DBM_MIN), 1)
	self:ScheduleAnnounce(255 - delay, DBM_GENERIC_ENRAGE_WARN:format(1, DBM_MIN), 2)
	self:ScheduleAnnounce(285 - delay, DBM_GENERIC_ENRAGE_WARN:format(30, DBM_SEC), 3)
	self:ScheduleAnnounce(305 - delay, DBM_GENERIC_ENRAGE_WARN:format(10, DBM_SEC), 4)
...
function Twins:OnEvent(event, args)
	if event == "SPELL_CAST_START" then
		...
		if args.spellId == 45342 then --Conflagration
			self:SendSync("Conflagration"..args.destName)
			self:StartStatusBarTimer(31, "~Next Conflagration", 45342)
			self:StartStatusBarTimer(3.5, "Conflagration", 45333)
		end
